<?php
require_once('tfpdf.php'); // Lee 11/19/21

class FPDF2File extends tFPDF // Lee 11/19/21
{
protected $f;
private $file;
...
function Open($file='')
{
	if(tFPDF_VERSION<'1.32') // Lee changed for tfpdf
		$this->Error('Version 1.8 or above is required by this extension');
    if ($file == '') { // Lee added for automatic filename creation
        $file = tempnam('/srv/spectra/htdocs/admintools' . BETAPATH . '/classes/tfpdf/temp/', false);
    }
    $this->f = fopen($file, 'wb');
    $this->file = $file; // Lee - save filename in class
	if(!$this->f)
...
function Output($dest='', $name='', $isUTF8=false) // note order has been changed
{
    if($this->state<3) $this->Close();

    if ($dest == '') {
        // Lee added 11/19/2021
