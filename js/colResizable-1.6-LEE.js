...
(function($){ 	
	...
	//append required CSS rules - Modified by Lee to remove some styles
	...
	*/
	var syncCols = function(t,i,isOver){
        ...
        if(t.f){ //if fixed mode
            ...
            t.width(t.w + inc);		// added by Lee to fix random adjustments of other cells than the one being changed
        }
		if(isOver){
            c.w=w;
            c2.w= t.f ? w2 : c2.w;
	...
	*/
		...
		if(t.opt.liveDrag){ 			//if liveDrag is enabled
			if(last){
                ...
                if(!t.f && t.opt.overflow){			//if overflow is set, incriment min-width to force overflow
					...
					t.width(t.w + x - drag.l);	// added by Lee to fix random adjustments of other cells than the one being changed
                }else {
                    t.w = t.width();
                }
			}else{
	...
	var onGripDragOver = function(e){	
        ...
        if (!(drag.x - drag.l == 0)) {
            ...
            if (cb) { e.currentTarget = t[0]; cb(e, i); }	//if there is a callback function, it is fired // modified by Lee to add passing of column index of modified column
